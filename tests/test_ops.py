...
class Shift_params_test(unittest.TestCase):
  ...
  def test_null(self):
    inval=self.inval
    inval.append(0)
    outval=swtools.shift_param(*inval)
    for i in range(4):
      f=lambda x:x
      if i>1:
        f=self.to_sec
  ...
  def test_ineq_len(self):#length arrays are different
    ...
    outval=swtools.shift_param(param1,param2,datet1,datet2,delta)
  ...
  def test_auto(self):
    h=3600
    out=swtools.shift_param(*self.inval,auto=True,return_delta=True,v=0,dt_lim=[-6*h,0])
    delta_t=out[-1]
...
class Concatenation_tests(unittest.TestCase):
  ...
  def test_1d(self):
    #len checks
    out1=swtools.sw_io.concatenate_values(self.d11,self.d12)
    nt.eq_(len(out1),(len(self.d11)+len(self.d12)),"Incorrect concatenation length {}!={}+{}"
      .format(len(out1),len(self.d11),len(self.d12)))
    
    out2=swtools.sw_io.concatenate_values(self.d11,self.d12,self.d11)
  ...
  def test_2d(self):
    ...
    out1=swtools.sw_io.concatenate_values(self.d21,self.d21*2,self.d21)
    out2=swtools.sw_io.concatenate_values(self.d21,self.d22*3)
    out3=swtools.sw_io.concatenate_values(self.d21,self.d23*4)
    out4=swtools.sw_io.concatenate_values(self.d21,self.d21*10,axis=0)
  ...
  def test_3d(self):
    out1=swtools.sw_io.concatenate_values(self.d31,3*self.d31,axis=0)
    out2=swtools.sw_io.concatenate_values(self.d31,self.d32)
    out3=swtools.sw_io.concatenate_values(self.d31,self.d33)
    out4=swtools.sw_io.concatenate_values(self.d31,self.d32,self.d33)
...
def test_where_overlap():
  ...
  out1=swtools.ops.where_overlap(t1,t2)
  out2=swtools.ops.where_overlap(t1,t3)
  out3=swtools.ops.where_overlap(t1,t2,delta)
  out4=swtools.ops.where_overlap(t1,t3,-delta)
...
def test_cyclic_rising():
  ...
  for N in [50,51]:
    for v0 in [0,pi/4,pi/3,pi/2-pi/10]:
      ...
      a_r = swtools.cyclic2rising(a_0,lim=[-A,A])
      a_c = swtools.rising2cyclic(a_r,lim=[-A,A])
...
def test_fourier_transform():
  ...
  a_fft,f_fft=swtools.fourier_transform(a_0,1)
...
def test_where_diff():
  ...
  out1=swtools.where_diff(a)[0]
  out2=swtools.where_diff(a,rtol=1)[0]
  out3=swtools.where_diff(a,atol=1)[0]
  out4=swtools.where_diff(a,rtol=1,atol=1,no_jump=True)[0]
  ...
  out5=swtools.where_diff(b,no_jump=True,axis=0)
  out6=swtools.where_diff(b,axis=1)
...
class Align_params_test(unittest.TestCase):
  ...
  def test_null(self):
    out  = swtools.align_param(*self.inval)
    atol = 1e-8
    for i in range(2):
  ...
  def test_sampling(self):
    ...
    out1 = swtools.align_param(p1,p2l,t1,t2l,align_to=True)#upsample
    out2 = swtools.align_param(p1,p2l,t1,t2l,align_to=False)#downsample
  ...
  def test_spline(self):
    ...
    tdiff = dt.timedelta(seconds=(inval[2][1]-inval[2][0]).total_seconds()/2)
    inval[2] = inval[2]-tdiff
    outk1 = swtools.align_param(*inval,k=1)
    outk3 = swtools.align_param(*inval,k=3)
