...
def getCDFparams(inloc,*params,**kwargs):
  """
  .. _getCDFparams:
  ...
  inloc : str
    ...
    cdf file. 
  params : str 
    Names of parameters to be extracted from `inloc`. If names are not
  ...
  -----------------
    param0 : str
      ...
      used unless ``filter_param=True`` is specified.
...
def _get_zip_content(zf,**zp):
  ...
  if zp['temp']:
    zp['outloc']=tempfile.mkdtemp(prefix="swtools_")
  z=unzip_file(zf, zp['outloc'])
  return [os.path.join(zp['outloc'],f) for f in z.namelist() 
    if f.upper().split('.')[-1] in zp['cdfsuffix']]


def getCDFlist(inloc=None, outloc=os.curdir,sort_by_t=False, **kwargs):
  """
  .. _getCDFlist:
  ...
  -------
    cdfl : list 
      list of strings of absolute paths to cdf files found in `inloc`.

  Notes
  -----
  The `inloc` argument will be overwritten by any `inloc` entry in 
  `kwargs`.
  ...
  if not os.path.exists(outloc):
    if len(os.path.basename(outloc).split('.'))<2:
      os.makedirs(outloc)
      aux.logger.debug("creating directory: '{}'".format(outloc))
  
  if inloc == None:
    inloc = os.curdir

  zp=dict(cdfsuffix=['DBL','CDF'],temp=False,use_ftp=False,includezip=False,
          inloc=inloc,outloc=outloc,filter_param=False,param0=None,sat=None,
  ...
  if not zp['use_ftp'] and 'user' not in zp:
    if isinstance(zp['inloc'],str):
      zp['inloc']=os.path.abspath(zp['inloc'])
    else:
      for i,zloc in enumerate(zp['inloc']):
        zp['inloc'][i]=os.path.abspath(zloc)
  zp['outloc']=os.path.abspath(zp['outloc'])
  aux.logger.debug('kwargs set to {}'.format(zp))
  #if inloc is directory: glob it for cdf's otherwise for zip's
  ...
  if isinstance(zp['inloc'],str):
    if zp['use_ftp'] or ('user' in zp):
        fl=aux._tolist(dl_ftp(zp['inloc'],**zp))
        if not fl:
          return []
        for fn in fl:
          cdflist+=_locate_cdfs(fn)
    else:
      cdflist=_locate_cdfs(zp['inloc'])
  else:
    for loc in zp['inloc']:
      if zp['use_ftp'] or ('user' in zp):
        fn=aux._tolist(dl_ftp(zp['inloc'],**zp))
        if fn:
          cdflist+=_locate_cdfs(fn)
      else:
...
def getCDFparamlist(cdflist,cdfsuffix=['DBL','CDF']):
    """
    .. _getCDFparamlist:
    ...
    cdfsuffix : str or list of str, optional 
        ...
        as valid cdf files (default ``['DBL','CDF']``).
...
def param_peek(in_arr_cdfl,parameter=None,n_show=5,axis=0,cataxis=None):
  ...
  Print values from a cdf file, list of cdf files or `numpy.ndarray`:
  ...
  Parameters
  ----------
    in_arr_cdfl : str or list of str or `numpy.ndarray`
  ...
  if dim==2:
    ...
    for i in range(sh_min[0]):
      peek_info.append(
        '{}/{}\tmax: {:8f},\tmin: {:8f},\tmean: {:8f},\tmedian: {:8f}'
        .format(i+1,sh_min[0],np.max(p_a_view[i]),np.min(p_a_view[i]),
          np.mean(p_a_view[i]),np.median(p_a_view[i]))) 
  else:
      peek_info.append(
        'max: {:8f},\tmin: {:8f},\tmean: {:8f},\tmedian: {:8f}'
        .format(np.max(p_arr),np.min(p_arr),np.mean(p_arr),np.median(p_arr)))
...
def dl_ftp(url='swarm-diss.eo.esa.int',**kwargs):
  ...
  pre='ftp://'
  #aux._importable('ftputil')
  ftpkw=dict( user=None,pw=None,use_color=True,dlloc=os.getcwd(),
              cdfsuffix=['DBL','CDF'],sat='',use_current=True,
              param0=None,period=None,use_filtering=True,
              use_passive_mode=True)
  #period=[dt0,dt1] #default:None
  #Sat=None can be str or list of str ie ['A'/'Sat_A','B'] etc
  #firstparam=None param which will decide what product to dl
  if 'outloc' in kwargs:
    ftpkw['dlloc']=kwargs['outloc']
  ftpkw.update(kwargs)
  ...
  with ftputil.FTPHost(baseurl, ftpkw['user'], ftpkw['pw']
    ...
    for fmt in ftpkw['cdfsuffix']+['.zip']:
      if url.endswith(fmt.lower()) or url.endswith(fmt.upper()):
        if host.path.isfile(host.path.basename(url)):
          ...
          if os.path.isfile(
              os.path.join(ftpkw['dlloc'],host.path.basename(url))):
            aux.logger.debug('File {} already downloaded.'
              .format(host.path.basename(url)))
            return os.path.join(ftpkw['dlloc'],host.path.basename(url))
    ...
    for s in selected:
      if host.path.isdir(s):
        ...
        for f in dir_content:
          ...
          if use_filtering:
            filter_filelist(dir_content,**ftpkw)
          f_dl += dir_content
      else:
        f_dl.append(s)
    ...
    for f in f_dl:
        ...
        localloc = os.path.abspath(os.path.join(
          ftpkw['dlloc'],host.path.basename(f)))
